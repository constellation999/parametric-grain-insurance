...
├── plots.py                           ← generates the three figures below
├── calibration.json                   ← calibrated frequencies, peaks, durations, lifts
├── sim_summary.json                   ← headline numbers
├── fig1_realdata_backtest.png         ← generated by plots.py (not committed)
├── fig2_montecarlo_solvency.png       ← generated by plots.py (not committed)
└── fig3_welfare_demand_funding.png    ← generated by plots.py (not committed)
```

Figures are produced by `python plots.py` after a successful `sim.py` run (requires the intermediate `.npy` files). They are also present in the original design package.
